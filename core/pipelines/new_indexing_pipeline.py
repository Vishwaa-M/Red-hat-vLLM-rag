...
from core.indexing_service.data_loaders.new_minio_document_processor import DocumentProcessor
...
def chunk_documents_node(state: GraphState) -> Dict[str, Any]:
    ...
    if not (text_docs or table_docs):
        logger.info("No documents to chunk, moving to embed documents")
        return {
            "chunked_documents": [],
...
def embed_documents_node(state: GraphState) -> Dict[str, Any]:
    """Generate dense and sparse embeddings for chunked documents."""
    step_count = state.get("step_count", 0) + 1
    logger.debug("Step %d: Generating embeddings", step_count)
    
    chunked_docs = state.get("chunked_documents", [])
    if not chunked_docs:
        logger.info("No chunks to embed, moving to store in Milvus")
        return {
            "embedded_documents": [],
    ...
    logger.info(f"Embedding {len(chunked_docs)} chunked documents")
    try:
        # Use chunked documents directly
        documents = [
            Document(
                ...
                metadata={
                    **doc.metadata,
                    ...
                    "parent_doc_id": doc.metadata.get("parent_doc_id", doc.metadata.get("doc_id", "unknown")),
...
def collect_results_node(state: GraphState) -> Dict[str, Any]:
    """Collect final results and save chunk metadata."""
    step_count = state.get("step_count", 0) + 1
    logger.info("Step %d: Collecting results: %d chunked, %d embedded, %d errors",
                step_count, len(state.get("chunked_documents", [])),
                len(state.get("embedded_documents", [])),
                len(state.get("error_log", [])))
    
    if state.get("chunked_documents"):
        try:
            chunk_metadata = [
                {
                ...
                for doc in state.get("chunked_documents", [])
            ]
            with open("chunks_for_extraction.json", "w") as f:
                json.dump(chunk_metadata, f, indent=2)
            logger.info("Saved chunk metadata to chunks_for_extraction.json")
        except Exception as e:
            logger.error("Failed to save chunk metadata: %s\n%s", str(e), traceback.format_exc())
...
def build_graph():
    ...
    graph_builder.add_edge("chunk_documents", "embed_documents")
...
def run_graph() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Pipeline completed: %d chunked, %d embedded, %d errors, %d steps taken",
                    len(chunked_docs), len(embedded_docs), len(errors), result.get("step_count", 0))
        return {
