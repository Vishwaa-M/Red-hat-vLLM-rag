...
from unstructured.documents.elements import Table, NarrativeText, ListItem
import boto3
...
class DocumentProcessor:
    ...
    Text is extracted as paragraphs, with bullet points and lists grouped as single elements.
    Tables are preserved in Markdown format for RAG compatibility.
    """
    ...
    def __init__(
        ...
        self.min_image_size = (100, 100)  # Minimum width, height in pixels for filtering
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.transform = T.Compose([])  # No resizing to preserve original quality
    ...
    def _process_image(self, image: Image.Image) -> Image.Image:
        try:
            # Convert to RGB if necessary and return as-is for original quality
            if image.mode != 'RGB':
        ...
        try:
            with zipfile.ZipFile(temp_file_path, 'r') as docx_zip:
                ...
                for file_info in media_files:
                    ...
                    if image_ext in ['png', 'jpg', 'jpeg', 'bmp', 'gif', 'tiff', 'emf', 'wmf']:
                        try:
    ...
    def _table_to_markdown(self, table_content: str, metadata: dict) -> str:
        """
        Clean and return table content in Markdown format, preserving terminal-style tables.
        Removes unnecessary separators and transaction summaries, ensuring valid Markdown.
        """
        try:
            # Clean and split content into lines
            lines = [line.strip() for line in table_content.split('\n') if line.strip()]
            if not lines:
                logger.debug("No valid table content")
                return ""

            # Initialize output lines
            cleaned_lines = []
            in_summary = False
            # Detect terminal-style table separators (e.g., ==== or ----)
            ...
            for line in lines:
                ...
                # Check for transaction summary or non-table content
                if "Transaction Summary" in line or "Is this ok" in line:
                    in_summary = True
                    continue
                ...
                # Preserve the line as-is for Markdown
                cleaned_lines.append(line)

            # Join lines to form Markdown table
            result = "\n".join(cleaned_lines).strip()
            if not result:
                logger.debug("Empty table after cleaning")
                return ""

            logger.debug("Markdown table extracted: %s", result[:100])
            return result
        except Exception as e:
            logger.error("Failed to process table to Markdown: %s", str(e))
            return ""
    ...
    async def load_and_parse_documents(self) -> Tuple[List[Document], List[Document], List[Document]]:
        ...
        Returns:
            Tuple[List[Document], List[Document], List[Document]]: Lists containing text, table, and image Document objects.
        """
    ...
    async def _parse_raw_file(self, content: bytes, source: str) -> Tuple[List[Tuple[str, str, dict]], List[Tuple[str, str, dict]], List[Tuple[str, str, dict]]]:
        ...
        try:
            ...
            try:
                ...
                # Track current paragraph or list
                current_text = ""
                ...
                for element, level in doc.iterate_items():
                    if isinstance(element, TextItem):
                        ...
                        element_label = str(element.label).lower()
                        is_list_item = element_label in ["list-item", "bullet", "list"]

                        new_metadata = self._sanitize_metadata({
                            "element_type": "List" if is_list_item else "Text",
                            "source": source,
                        ...
                        if is_list_item:
                            if not is_list and current_text:
                                text_elements.append((current_text.strip(), source, current_metadata))
                                current_text = ""
                                current_metadata = None
                            ...
                            if is_list and current_text:
                                text_elements.append((current_text.strip(), source, current_metadata))
                                current_text = ""
                                current_metadata = None
                                is_list = False
                            if current_text and last_element_type == "Text" and current_metadata["page_number"] == page_num:
                                current_text += " " + content
                                current_metadata["char_count"] += len(content)
                            else:
                                if current_text:
                                    text_elements.append((current_text.strip(), source, current_metadata))
                                current_text = content
                        ...
                        markdown_content = self._table_to_markdown(content, metadata)
                        if markdown_content:
                            table_elements.append((markdown_content, source, metadata))
                            table_count += 1
                            logger.debug("Extracted Docling table on page %s: %s", metadata["page_number"], markdown_content[:100])
                ...
                try:
                    ...
                    for element in unstructured_elements:
                        ...
                        content = str(element.text) if hasattr(element, "text") else ""
                        ...
                        if element.category == "ListItem":
                            ...
                            is_list = True
                            if not current_metadata:
                                current_metadata = self._sanitize_metadata({
                                    "element_type": "List",
                                    "source": source,
                                    "page_number": page_num,
                                    "content_layer": "BODY",
                        ...
                        elif element.category == "NarrativeText" and content.strip():
                            if is_list and current_text:
                                text_elements.append((current_text.strip(), source, current_metadata))
                                current_text = ""
                                current_metadata = None
                            ...
                            if current_text:
                                ...
                                last_element_type = None
                            if not content.strip():
                                logger.debug("Skipping empty table element on page %s", page_num)
                                continue
                            ...
                            if markdown_content:
                                table_elements.append((markdown_content, source, metadata))
                                logger.debug("Extracted Unstructured.io table on page %s: %s", metadata["page_number"], markdown_content[:100])
                            ...
                            try:
                                if hasattr(element.metadata, 'image_base64') and element.metadata.image_base64:
                                    logger.debug("Found image_base64 for page %s", page_num)
                                    image_data = base64.b64decode(element.metadata.image_base64)
                                    image = Image.open(io.BytesIO(image_data))
                                    image = self._process_image(image)
                                    width, height = image.size
                                    if width < self.min_image_size[0] or height < self.min_image_size[1]:
                                        logger.debug("Skipping small Unstructured.io image on page %s: %dx%d", page_num, width, height)
                                        continue
                                    if self.store_images_on_disk:
                                    ...
                                    logger.warning("No image_base64 found for image element on page %s", page_num)
                            except Exception as img_error:
                                logger.error("Failed to decode Unstructured.io image on page %s: %s", page_num, str(img_error))
                            metadata = self._sanitize_metadata({
                                "element_type": "Image",
                                "source": source,
                                "page_number": page_num,
                                ...
                                "caption": self._find_caption(unstructured_elements, page_num, "Figure"),
